...
Apply migrations in numeric order through `014_inventory_traceability.sql`. Production inventory, lot, stock-count, commissioning and device-lifecycle tables are intentionally empty after migration; warehouse counts, supplier lots, module/device serial numbers, calibration records and physical installation records must come from approved physical checks or controlled import. Reliability job definitions are registered by the application, while run and incident rows are created only by actual executions.
