...
This workflow is a control-plane record, not a service certificate. Production must connect its actor IDs to OIDC principals and validate the process through repeated live field cycles. SQLite reports remediation migration `2026-08-28.remediation-dispatch-sla-v1` and operations-control migration `2026-08-29.ops-control-import-v1`; PostgreSQL deployments must apply `infra/postgres/003_remediation_tasks.sql` through `007_ops_control_import.sql`, then pass preflight migration/table checks.
...
The current apply loop is recoverable and idempotent but is not one cross-store database transaction: a process interruption can require replaying the same preview batch. Production PostgreSQL rollout should move work-order upserts, batch status and audit insertion into one reviewed transaction before treating high-volume imports as unattended automation.
