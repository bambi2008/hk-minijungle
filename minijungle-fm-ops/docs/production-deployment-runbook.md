...
The module queue can create and update internal remediation tasks. `POST /api/remediation/tasks` accepts a scoped `moduleId`, `sourceKey`, `reasons`, `priority`, optional `assignedTo` and optional `dueAt`; repeated active creation for the same module/source is idempotent. `PATCH /api/remediation/tasks/:id` moves a task through `open`, `assigned`, `in_progress`, `resolved` or `cancelled`. `assigned` and `in_progress` require an assignee; `resolved` requires a resolution note or evidence reference. FM Lead can manage tasks within scope, while a field technician can update only a task assigned to that technician. Each transition is appended to the operations timeline as `remediation.task.<status>`.

This workflow is a control-plane record, not a service certificate. Production must connect the task to the real work-order ID, OIDC principal, mobile acknowledgement, device/camera evidence and independent closure review. The SQLite pilot migration is `2026-08-25.remediation-tasks-v1`; production must apply `infra/postgres/003_remediation_tasks.sql` and confirm the preflight migration/table checks before enabling this path.

The technician PWA reads `GET /api/mobile/remediation-tasks?statuses=open,assigned,in_progress` and updates `PATCH /api/mobile/remediation-tasks/:id`. A field technician receives only tasks assigned to that principal; a successful mobile capture may resolve the task using the capture batch ID or uploaded media ID as `evidenceRef`. This is still a pilot control flow until the task is linked to the production work-order dispatch record and independently reviewed.
