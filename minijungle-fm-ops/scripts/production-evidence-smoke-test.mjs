...
const pilot = buildProductionEvidenceReport({
  ...
  storage: { backend: "sqlite", masterData: { backend: "sqlite" }, mobileCapture: { backend: "sqlite" }, proofMedia: { backend: "sqlite", storageProvider: "local" }, telemetry: { backend: "sqlite" }, modules: { backend: "sqlite" }, reminders: { backend: "sqlite" }, devices: { backend: "sqlite" }, alerts: { backend: "sqlite" }, aiVision: { backend: "sqlite" } }
});
...
const production = buildProductionEvidenceReport({
  ...
  storage: { backend: "postgresql", masterData: { backend: "postgresql" }, mobileCapture: { backend: "postgresql" }, proofMedia: { backend: "postgresql", storageProvider: "s3-compatible" }, telemetry: { backend: "postgresql" }, modules: { backend: "postgresql" }, reminders: { backend: "postgresql" }, devices: { backend: "postgresql" }, alerts: { backend: "postgresql" }, aiVision: { backend: "postgresql" } }
});
