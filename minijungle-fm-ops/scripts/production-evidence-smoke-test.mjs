...
const production = buildProductionEvidenceReport({
  ...
  storage: { backend: "postgresql", masterData: { backend: "postgresql" }, mobileCapture: { backend: "postgresql" }, proofMedia: { backend: "postgresql", storageProvider: "s3-compatible" }, telemetry: { backend: "postgresql" }, modules: { backend: "postgresql" }, reminders: { backend: "postgresql" }, remediation: { backend: "postgresql" }, devices: { backend: "postgresql" }, alerts: { backend: "postgresql" }, aiVision: { backend: "postgresql" }, notifications: { backend: "postgresql" }, evidenceSnapshots: { backend: "postgresql" }, integrations: { backend: "postgresql" }, workforce: { backend: "postgresql" }, maintenancePlanning: { backend: "postgresql" } }
});
