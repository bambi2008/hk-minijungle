...
async function main() {
  ...
  try {
    ...
    try {
      ...
      await operations.goto(`${baseUrl}/operations.html`); await operations.waitForFunction(() => document.querySelector("#open-count")?.textContent !== "--", null, { timeout: 30000 });
      ...
      await operations.waitForFunction(() => document.querySelector("#module-query-state")?.textContent.includes("1 of 1 modules"), null, { timeout: 30000 });
      ...
      await operations.waitForFunction(() => document.querySelector("#contract-notice")?.textContent.includes("created as draft"), null, { timeout: 30000 });
      ...
      await operations.waitForFunction(() => document.querySelector("#contract-notice")?.textContent.includes("now active"), null, { timeout: 30000 });
      ...
      await operations.waitForFunction(() => document.querySelector("#contract-change-list")?.textContent.includes("DF-HK-UI-001"), null, { timeout: 30000 });
      ...
      await operations.waitForFunction(() => document.querySelector("#contract-notice")?.textContent.includes("updated"), null, { timeout: 30000 });
      await operations.waitForFunction(() => [...document.querySelectorAll(".contract-item")].some((item) => item.textContent.includes("Vision Care UI Plus")), null, { timeout: 30000 });
      ...
      await operations.waitForFunction(() => document.querySelector("#inventory-notice")?.textContent.includes("loaded to route kit"), null, { timeout: 30000 });
      ...
      await operations.waitForFunction(() => document.querySelector("#maintenance-notice")?.textContent.includes("phone route"), null, { timeout: 30000 });
      await operations.waitForFunction(() => document.querySelector("#maintenance-plan-list")?.textContent.includes("field-tech-show-suite"), null, { timeout: 30000 });
      ...
      await operations.waitForFunction(() => { const text = document.querySelector("#maintenance-import-notice")?.textContent || ""; return text.includes("1 maintenance row imported") || (text.trim() && !text.includes("Applying validated maintenance rows")); }, null, { timeout: 60000 });
      assert((await operations.locator("#maintenance-import-notice").textContent()).includes("1 maintenance row imported"), `Maintenance import apply failed: ${await operations.locator("#maintenance-import-notice").textContent()}\nServer output:\n${serverOutput.join("")}`);
      ...
      await operations.waitForSelector("[data-remediation-edit]", { timeout: 30000 });
      ...
      await operations.waitForFunction(() => document.querySelector("#dispatch-notice")?.textContent.includes("1 task updated"), null, { timeout: 30000 });
      await operations.locator("#dispatch-sla-scan").click();
      await operations.waitForFunction(() => document.querySelector("#dispatch-notice")?.textContent.includes("1 newly escalated"), null, { timeout: 30000 });
      await operations.waitForFunction(() => document.querySelector("#dispatch-list")?.textContent.includes("overdue"), null, { timeout: 30000 });
      ...
      await operations.waitForFunction(() => document.querySelector("#reliability-notice")?.textContent.includes("Check complete"), null, { timeout: 30000 });
      ...
      await operations.locator("#remediation-submit").click();
      await operations.waitForFunction(() => { const timeline = document.querySelector("#timeline-list")?.textContent || ""; const notice = document.querySelector("#notice")?.textContent || ""; return timeline.includes("remediation.task.review-approved") || notice.includes("Failed to fetch"); }, null, { timeout: 30000 });
      assert((await operations.locator("#timeline-list").textContent()).includes("remediation.task.review-approved"), `Remediation approval refresh failed: ${await operations.locator("#notice").textContent()}\nServer output:\n${serverOutput.join("")}`);
      ...
      await operations.locator('[data-ai-start="AIV-UI-001"]').click(); await operations.waitForSelector('[data-ai-review="AIV-UI-001"]', { timeout: 30000 });
      ...
      await operations.locator('[data-ai-review="AIV-UI-001"]').click(); await operations.locator("#ai-review-confidence").fill("0.88"); await operations.locator("#ai-review-note").fill("Provider result reviewed against fixed-angle capture; schedule a follow-up photo."); await operations.locator("#ai-review-submit").click(); await operations.waitForSelector('[data-ai-review="AIV-UI-001"]', { state: "detached", timeout: 30000 });
      ...
      await operations.locator("#persist-snapshot").click(); await operations.waitForFunction(() => document.querySelector("#snapshot-notice")?.textContent.includes("persisted"), null, { timeout: 30000 }); await operations.waitForFunction(() => document.querySelector("#snapshot-state")?.textContent.includes("1 stored"), null, { timeout: 30000 }); await operations.waitForFunction(() => document.querySelector("#timeline-list")?.textContent.includes("evidence.snapshot.persisted"), null, { timeout: 30000 });
      ...
      await operations.locator("#verify-snapshot").click(); await operations.waitForFunction(() => document.querySelector("#snapshot-notice")?.textContent.includes("verification:"), null, { timeout: 30000 });
      await operations.locator("#sweep-snapshots").click(); await operations.waitForFunction(() => document.querySelector("#snapshot-notice")?.textContent.includes("Retention sweep complete"), null, { timeout: 30000 });
      ...
      await mobile.goto(`${baseUrl}/mobile.html`); await mobile.evaluate(() => localStorage.removeItem("dr-forest.field-capture.queue.v3")); await mobile.waitForFunction(() => document.querySelectorAll(".stop").length > 0, null, { timeout: 30000 }); await mobile.locator(".stop").first().click(); await mobile.waitForTimeout(300);
      ...
      await mobile.waitForFunction(() => document.querySelector("#commissioning-mobile-state")?.textContent === "PLANNED", null, { timeout: 30000 });
      ...
      await mobile.waitForFunction(() => document.querySelector("#sync-state")?.textContent.includes("calibrated recorded"), null, { timeout: 30000 });
      ...
      await mobile.waitForFunction(() => document.querySelector("#commissioning-mobile-state")?.textContent === "INSTALLED", null, { timeout: 30000 });
      ...
      await mobile.waitForSelector(".remediation-item", { timeout: 30000 });
      ...
      await mobile.waitForFunction(() => document.querySelector("#remediation-list")?.textContent.includes("in progress"), null, { timeout: 30000 });
      ...
      await mobile.waitForFunction(() => document.querySelector("#remediation-list")?.textContent.includes("Awaiting FM review"), null, { timeout: 30000 });
      await mobile.waitForFunction(() => document.querySelector("#route-kit-list")?.textContent.includes("490 ml available") && document.querySelector("#sync-state")?.textContent !== "Loading", null, { timeout: 30000 });
      ...
      await mobile.waitForFunction(() => document.querySelector("#sync-state")?.textContent.includes("count submitted"), null, { timeout: 30000 });
      await operations.setViewportSize({ width: 1280, height: 900 });
      await operations.reload(); await operations.waitForFunction(() => document.querySelector("#inventory-trace-state")?.textContent.includes("1 count review"), null, { timeout: 30000 });
      await operations.locator(".inventory-trace").evaluate((element) => { element.open = true; });
      ...
      await operations.waitForFunction(() => { const notice = document.querySelector("#inventory-notice")?.textContent || ""; return notice.includes("approved") || (notice.trim() && !notice.includes("Applying counted variance")); }, null, { timeout: 30000 });
      assert((await operations.locator("#inventory-notice").textContent()).includes("approved"), `Inventory count review failed: ${await operations.locator("#inventory-notice").textContent()}\nServer output:\n${serverOutput.join("")}`);
      ...
      await mobile.reload();
      await mobile.waitForFunction(() => document.querySelector("#remediation-list")?.textContent.includes("No assigned module tasks"), null, { timeout: 30000 });
      await mobile.waitForFunction(() => document.querySelectorAll(".stop").length > 0, null, { timeout: 30000 });
      await mobile.locator(".stop").first().click();
      await mobile.waitForFunction(() => document.querySelector("#notes")?.value === "" && document.querySelector("#exception")?.checked === false, null, { timeout: 30000 });
      ...
      await mobile.route("**/api/mobile/*", (request) => request.abort()); await mobile.reload({ waitUntil: "domcontentloaded" }); await mobile.waitForFunction(() => document.querySelector("#sync-state")?.textContent.toLowerCase().includes("cached"), null, { timeout: 30000 });
      ...
      await clientPortal.goto(`${baseUrl}/portal.html?role=client`); await clientPortal.waitForFunction(() => document.querySelector("#asset-state")?.textContent !== "Loading", null, { timeout: 30000 });
      ...
      assert(await clientPortal.locator("#feedback-form-panel").isVisible(), "Client portal did not expose the feedback submission form");
      await clientPortal.locator("#feedback-service-ref").fill("WO-1047");
      await clientPortal.locator("#feedback-rating").selectOption("4");
      await clientPortal.locator("#feedback-outcome").selectOption("follow_up_required");
      await clientPortal.locator("#feedback-follow-up").check();
      await clientPortal.locator("#feedback-comment").fill("UI smoke feedback: please confirm the next plant-care visit window.");
      await clientPortal.locator("#feedback-submit").click();
      await clientPortal.waitForFunction(() => document.querySelector("#feedback-notice")?.textContent.includes("Feedback received"), null, { timeout: 30000 });
      await clientPortal.waitForFunction(() => document.querySelector("#feedback-list")?.textContent.includes("WO-1047"), null, { timeout: 30000 });
      const clientFeedbackListText = await clientPortal.locator("#feedback-list").textContent();
      assert(clientFeedbackListText.includes("WO-1047"), `Client portal did not render the submitted feedback: state=${await clientPortal.locator("#feedback-state").textContent()} notice=${await clientPortal.locator("#feedback-notice").textContent()} list=${clientFeedbackListText}\nServer output:\n${serverOutput.join("")}`);
      ...
      await operations.reload();
      await operations.waitForFunction(() => document.querySelector("#client-feedback-state")?.textContent !== "Loading", null, { timeout: 30000 });
      assert(await operations.locator(".client-feedback-item").count() === 1, "FM Today did not render the submitted client feedback");
      assert((await operations.locator("#client-feedback-list").textContent()).includes("follow up required"), "FM Today did not preserve the feedback follow-up state");
      const feedbackItem = operations.locator(".client-feedback-item").first();
      await feedbackItem.locator("[data-client-feedback-note]").fill("FM reviewed the visit and scheduled the follow-up window.");
      await feedbackItem.locator("[data-client-feedback-review]").click();
      await operations.waitForFunction(() => document.querySelector("#client-feedback-list")?.textContent.includes("acknowledged"), null, { timeout: 30000 });
      const acknowledgedFeedback = operations.locator(".client-feedback-item").first();
      await acknowledgedFeedback.locator("[data-client-feedback-note]").fill("Follow-up window confirmed with the client.");
      await acknowledgedFeedback.locator("[data-client-feedback-review]").click();
      await operations.waitForFunction(() => document.querySelector("#client-feedback-list")?.textContent.includes("closed"), null, { timeout: 30000 });
      assert((await operations.locator("#client-feedback-state").textContent()).includes("0 open"), "FM Today did not close the client feedback loop");
      ...
      await auditorPortal.goto(`${baseUrl}/portal.html?role=auditor`); await auditorPortal.waitForFunction(() => document.querySelector("#quality-status")?.textContent !== "Loading", null, { timeout: 30000 });
      ...
      await admin.goto(`${baseUrl}/admin.html`); await admin.waitForFunction(() => document.querySelectorAll("#clients-list tbody tr").length === 4, null, { timeout: 30000 }); await admin.locator("[data-tab=modules]").click();
      ...
      await admin.locator('#editor-form button[type="submit"]').click(); await admin.waitForFunction(() => document.querySelector("#notice")?.textContent.includes("service profile"), null, { timeout: 30000 });
      ...
      await admin.waitForFunction((moduleId) => [...document.querySelectorAll("#commissioning-list tbody tr")].some((row) => row.textContent.includes(moduleId) && row.textContent.includes("VERIFIED")), mobileSeed.moduleId, { timeout: 30000 });
