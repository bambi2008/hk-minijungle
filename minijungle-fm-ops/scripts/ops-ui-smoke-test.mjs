...
async function main() {
  ...
  try {
    ...
    try {
      ...
      const operationsAssign = await fetch(`${baseUrl}/api/remediation/tasks/${encodeURIComponent(operationsRemediationId)}`, { method: "PATCH", headers: { "Content-Type": "application/json", "x-dr-forest-principal": "fm-lead" }, body: JSON.stringify({ status: "assigned", assignedTo: "field-tech-show-suite" }) });
      assert(operationsAssign.ok, "UI smoke could not assign the operations remediation task");
