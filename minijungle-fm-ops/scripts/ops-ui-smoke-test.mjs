...
async function main() {
  ...
  try {
    ...
    try {
      ...
      await operations.locator("[data-remediation-create]").first().click();
      ...
      await operations.locator("#remediation-status").selectOption("resolved");
      await operations.locator("#remediation-resolution-note").fill("Re-seated monitoring gateway and confirmed the next service check.");
      await operations.locator("#remediation-evidence-ref").fill("CAP-UI-REMEDIATION-001");
      await operations.locator("#remediation-submit").click();
      await operations.waitForFunction(() => document.querySelector("#timeline-list")?.textContent.includes("remediation.task.resolved"), null, { timeout: 10000 });
      assert(await operations.locator("[data-remediation-create]").count() === 12, "Resolved remediation task did not leave the active queue");
      ...
      const mobileTaskAssign = await fetch(`${baseUrl}/api/remediation/tasks/${encodeURIComponent(mobileTaskBody.task.id)}`, { method: "PATCH", headers: { "Content-Type": "application/json", "x-dr-forest-principal": "fm-lead" }, body: JSON.stringify({ status: "assigned", assignedTo: "field-tech-show-suite" }) });
      assert(mobileTaskAssign.ok, "UI smoke could not assign the mobile remediation task");
