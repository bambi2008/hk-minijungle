...
async function main() {
  ...
  try {
    ...
    try {
      ...
      await operations.locator("#persist-snapshot").click(); await operations.waitForFunction(() => document.querySelector("#snapshot-notice")?.textContent.includes("persisted"), null, { timeout: 5000 }); await operations.waitForFunction(() => document.querySelector("#snapshot-state")?.textContent.includes("1 stored"), null, { timeout: 5000 }); await operations.waitForFunction(() => document.querySelector("#timeline-list")?.textContent.includes("evidence.snapshot.persisted"), null, { timeout: 5000 });
