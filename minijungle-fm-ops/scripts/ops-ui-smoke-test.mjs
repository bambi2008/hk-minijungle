...
async function main() {
  ...
  try {
    ...
    try {
      ...
      await operations.waitForSelector("[data-remediation-edit]", { timeout: 5000 });
      ...
      await operations.locator("#remediation-submit").click();
      await operations.waitForFunction(() => document.querySelector("#timeline-list")?.textContent.includes("remediation.task.resolved"), null, { timeout: 5000 });
      ...
      const mobileTaskAssign = await fetch(`${baseUrl}/api/remediation/tasks/${encodeURIComponent((await mobileTaskCreate.json()).task.id)}`, { method: "PATCH", headers: { "Content-Type": "application/json", "x-dr-forest-principal": "fm-lead" }, body: JSON.stringify({ status: "assigned", assignedTo: "field-tech-show-suite" }) });
      ...
      await mobile.waitForSelector(".remediation-item", { timeout: 5000 });
      ...
      await mobile.waitForFunction(() => document.querySelector("#remediation-list")?.textContent.includes("in progress"), null, { timeout: 5000 });
      ...
      await mobile.waitForFunction(() => document.querySelector("#remediation-list")?.textContent.includes("No assigned module tasks"), null, { timeout: 5000 });
      await mobile.waitForFunction(() => document.querySelector("#notes")?.value === "" && document.querySelector("#exception")?.checked === false, null, { timeout: 5000 });
