...
async function verifyBrowserFlow(baseUrl) {
  ...
  try {
    ...
    await page.evaluate(() => localStorage.removeItem("dr-forest-fm-ops.language.v1"));
    ...
    assert(navText?.includes("Guide"), "English navigation did not load");
    assert(!navText?.includes("导读") && !navText?.includes("導讀"), "English navigation should not show Chinese labels");
    await page.click('[data-language-switch="zh-Hant"]');
    await page.waitForTimeout(150);
    const navTextZh = await page.textContent(".side-nav nav");
    assert(navTextZh?.includes("導讀"), "Traditional Chinese navigation did not load");
    const reportTabsZh = await page.textContent("#report-tabs");
    assert(reportTabsZh?.includes("機器人看護"), "Traditional Chinese report tabs did not load");
    const aiTextZh = await page.textContent("#ai");
    assert(aiTextZh?.includes("人工在環"), "Traditional Chinese AI governance content did not load");
    const healthTextZh = await page.textContent("#health");
    assert(healthTextZh?.includes("手機照片採集"), "Traditional Chinese health source content did not load");
    const roboticTextZh = await page.textContent("#robotic");
    assert(roboticTextZh?.includes("機器人採集"), "Traditional Chinese robotic care content did not load");
    await page.click('[data-language-switch="en"]');
    await page.waitForTimeout(150);
    const navTextBackToEn = await page.textContent(".side-nav nav");
    assert(navTextBackToEn?.includes("Guide"), "Switching back to English did not restore English navigation");
    const guideText = await page.textContent("#guide");
    assert(guideText?.includes("How to Read This Demo"), "English guide did not load");
    assert(guideText?.includes("English"), "Language guide note did not load");
    ...
    const roboticText = await page.textContent("#robotic");
    assert(roboticText?.includes("Robotic Care Layer"), "Robotic Care Layer section did not load");
    assert(roboticText?.includes("Autonomous patrol"), "Robotic care capabilities did not load autonomous patrol");
    assert(roboticText?.includes("Water refill"), "Robotic water refill capability did not load");
    assert(roboticText?.includes("Nutrient dosing"), "Robotic nutrient dosing capability did not load");
    assert(roboticText?.includes("Robot-readable module ID"), "Robotic wall interface readiness did not load module ID");
    const esgText = await page.textContent("#esg");
    assert(esgText?.includes("ESG Statistics"), "English ESG section did not load");
    ...
    assert(esgText?.includes("Robotic care readiness evidence"), "ESG proof pack did not load robotic care evidence");
    ...
    assert(reportTabsText?.includes("Robotic Care"), "Report Center did not load Robotic Care report mode");
...
async function main() {
  ...
  try {
    ...
    await verifyResource(baseUrl, "/data/robotic-care.json", "application/json");
