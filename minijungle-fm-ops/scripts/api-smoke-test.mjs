...
async function verifyApi(baseUrl) {
  ...
  const clientCommissioning = await fetchJson(`${baseUrl}api/commissioning`, { headers: principalHeaders("client-show-suite") });
  assert(clientCommissioning.response.ok && clientCommissioning.body.records.length === 3 && clientCommissioning.body.records.every((record) => record.clientId === "show-suite"), "Client commissioning view should remain read-only and client scoped");
  assert(clientCommissioning.body.summary.unplanned === 3, "Generated show-suite modules should remain visibly unplanned before physical commissioning");
  const clientPlanDenied = await fetchJson(`${baseUrl}api/commissioning`, { method: "POST", headers: { ...jsonHeaders("client-show-suite"), "Idempotency-Key": "commissioning-client-denied" }, body: JSON.stringify({ moduleId: "MJ-HK-021-M01", serialNumber: "DRF-SN-HK-021-01", publicCode: "DRF-HK-021-01" }) });
  assert(clientPlanDenied.response.status === 403, "Client viewer should not plan physical module commissioning");
  const missingCommissioningKey = await fetchJson(`${baseUrl}api/commissioning`, { method: "POST", headers: jsonHeaders("fm-lead"), body: JSON.stringify({ moduleId: "MJ-HK-021-M01", serialNumber: "DRF-SN-HK-021-01", publicCode: "DRF-HK-021-01" }) });
  assert(missingCommissioningKey.response.status === 428, "Commissioning plan should require an idempotency key");
  const plannedCommissioning = await fetchJson(`${baseUrl}api/commissioning`, { method: "POST", headers: { ...jsonHeaders("fm-lead"), "Idempotency-Key": "commissioning-plan-api-001" }, body: JSON.stringify({ moduleId: "MJ-HK-021-M01", serialNumber: "DRF-SN-HK-021-01", publicCode: "DRF-HK-021-01", hardwareRevision: "A1", installLocation: "Lift lobby bay 01" }) });
  assert(plannedCommissioning.response.status === 201 && plannedCommissioning.body.record.status === "planned", "FM Lead should assign a unique physical identity to a module");
  const duplicateCommissioningPlan = await fetchJson(`${baseUrl}api/commissioning`, { method: "POST", headers: { ...jsonHeaders("fm-lead"), "Idempotency-Key": "commissioning-plan-api-001" }, body: JSON.stringify({ moduleId: "MJ-HK-021-M01", serialNumber: "CHANGED-SERIAL", publicCode: "CHANGED-CODE" }) });
  assert(duplicateCommissioningPlan.response.ok && duplicateCommissioningPlan.body.duplicate === true && duplicateCommissioningPlan.body.record.serialNumber === "DRF-SN-HK-021-01", "Commissioning plan replay should return the first persisted identity");
  ...
  const installedCommissioning = await fetchJson(`${baseUrl}api/commissioning/MJ-HK-021-M01`, { method: "PATCH", headers: { ...jsonHeaders("field-tech-show-suite"), "Idempotency-Key": "commissioning-install-api-001" }, body: JSON.stringify({ toStatus: "installed", expectedUpdatedAt: plannedCommissioning.body.record.updatedAt, checklist: installChecklist, note: "Physical installation checks completed on site." }) });
  assert(installedCommissioning.response.ok && installedCommissioning.body.record.status === "installed" && installedCommissioning.body.record.installedBy === "field-tech-show-suite", "Assigned field technician should record attributable installation checks");
  const clientVerifyDenied = await fetchJson(`${baseUrl}api/commissioning/MJ-HK-021-M01`, { method: "PATCH", headers: { ...jsonHeaders("client-show-suite"), "Idempotency-Key": "commissioning-client-verify-denied" }, body: JSON.stringify({ toStatus: "verified", expectedUpdatedAt: installedCommissioning.body.record.updatedAt, checklist: { ...installChecklist, deviceMappingChecked: true, cameraViewChecked: true } }) });
  assert(clientVerifyDenied.response.status === 403, "Client viewer should not verify commissioning");
  const verifiedCommissioning = await fetchJson(`${baseUrl}api/commissioning/MJ-HK-021-M01`, { method: "PATCH", headers: { ...jsonHeaders("fm-lead"), "Idempotency-Key": "commissioning-verify-api-001" }, body: JSON.stringify({ toStatus: "verified", expectedUpdatedAt: installedCommissioning.body.record.updatedAt, checklist: { ...installChecklist, deviceMappingChecked: true, cameraViewChecked: true }, note: "FM Lead independently accepted identity, safety and device mapping." }) });
  assert(verifiedCommissioning.response.ok && verifiedCommissioning.body.record.status === "verified" && verifiedCommissioning.body.record.verifiedBy === "fm-lead", "Independent FM commissioning verification should persist reviewer attribution");
  const staleCommissioning = await fetchJson(`${baseUrl}api/commissioning/MJ-HK-021-M01`, { method: "PATCH", headers: { ...jsonHeaders("fm-lead"), "Idempotency-Key": "commissioning-stale-api-001" }, body: JSON.stringify({ toStatus: "suspended", expectedUpdatedAt: installedCommissioning.body.record.updatedAt, note: "Stale page must not suspend the module." }) });
  assert(staleCommissioning.response.status === 409 && staleCommissioning.body.code === "COMMISSIONING_VERSION_CONFLICT", "Stale commissioning page should not overwrite a newer lifecycle state");
  const mobileModuleCode = await fetchJson(`${baseUrl}api/mobile/module-code/DRF-HK-021-01`, { headers: principalHeaders("field-tech-show-suite") });
  assert(mobileModuleCode.response.ok && mobileModuleCode.body.record.moduleId === "MJ-HK-021-M01" && mobileModuleCode.body.record.status === "verified", "Technician phone should resolve an active physical module code within scope");
  const clientModuleCodeDenied = await fetchJson(`${baseUrl}api/mobile/module-code/DRF-HK-021-01`, { headers: principalHeaders("client-show-suite") });
  assert(clientModuleCodeDenied.response.status === 403, "Client viewer should not use the technician module-code route");
  const commissioningHistory = await fetchJson(`${baseUrl}api/commissioning/MJ-HK-021-M01/events`, { headers: principalHeaders("esg-auditor") });
  assert(commissioningHistory.response.ok && commissioningHistory.body.events.length === 3 && commissioningHistory.body.events[0].toStatus === "verified", "Auditor should read the immutable plan, install and verification history");
  const storageAfterCommissioning = await fetchJson(`${baseUrl}api/storage`, { headers: principalHeaders("fm-lead") });
  assert(storageAfterCommissioning.body.commissioning.counts.records === 1 && storageAfterCommissioning.body.commissioning.counts.verified === 1 && storageAfterCommissioning.body.commissioning.counts.events === 3, "Commissioning storage health should reflect one independently verified physical module");
  assert(storageAfterCommissioning.body.commissioning.relationshipIntegrity.foreignKeyIssues === 0, "Commissioning lifecycle introduced foreign-key issues");

  const clientDeviceLifecycleDenied = await fetchJson(`${baseUrl}api/device-lifecycle`, { headers: principalHeaders("client-show-suite") });
  assert(clientDeviceLifecycleDenied.response.status === 403, "Client viewer should not read internal device service controls");
  const fieldDeviceLifecycleNeedsModule = await fetchJson(`${baseUrl}api/device-lifecycle`, { headers: principalHeaders("field-tech-show-suite") });
  assert(fieldDeviceLifecycleNeedsModule.response.status === 400 && fieldDeviceLifecycleNeedsModule.body.code === "DEVICE_LIFECYCLE_MODULE_REQUIRED", "Technician lifecycle query should require assigned module scope");
  const lifecycleInitial = await fetchJson(`${baseUrl}api/device-lifecycle?moduleId=MJ-HK-021-M01`, { headers: principalHeaders("fm-lead") });
  const lifecycleDevice = lifecycleInitial.body.records.find((record) => record.type === "temperature");
  assert(lifecycleDevice && lifecycleDevice.profileStatus === "unmanaged", "Registered sensor should remain visibly unmanaged until a physical service profile is created");
  const missingLifecycleKey = await fetchJson(`${baseUrl}api/device-lifecycle/${encodeURIComponent(lifecycleDevice.deviceId)}/profile`, { method: "PUT", headers: jsonHeaders("fm-lead"), body: JSON.stringify({ serialNumber: "TEMP-HK-021-001", calibrationIntervalDays: 180 }) });
  assert(missingLifecycleKey.response.status === 428, "Device lifecycle profile should require an idempotency key");
  const lifecycleProfile = await fetchJson(`${baseUrl}api/device-lifecycle/${encodeURIComponent(lifecycleDevice.deviceId)}/profile`, { method: "PUT", headers: { ...jsonHeaders("fm-lead"), "Idempotency-Key": "device-profile-api-001" }, body: JSON.stringify({ serialNumber: "TEMP-HK-021-001", manufacturer: "DR FOREST", model: "TEMP-V1", calibrationIntervalDays: 180, lastCalibratedAt: "2026-01-01T00:00:00.000Z", note: "Imported physical sensor identity." }) });
  assert(lifecycleProfile.response.status === 201 && lifecycleProfile.body.record.calibrationState === "due", "FM Lead should create a due physical calibration profile");
  const fieldLifecycle = await fetchJson(`${baseUrl}api/device-lifecycle?moduleId=MJ-HK-021-M01`, { headers: principalHeaders("field-tech-show-suite") });
  assert(fieldLifecycle.response.ok && fieldLifecycle.body.records.some((record) => record.deviceId === lifecycleDevice.deviceId), "Assigned technician should read device controls only through the active module route");
  const technicianCalibration = await fetchJson(`${baseUrl}api/device-lifecycle/${encodeURIComponent(lifecycleDevice.deviceId)}/actions`, { method: "POST", headers: { ...jsonHeaders("field-tech-show-suite"), "Idempotency-Key": "device-calibration-api-001" }, body: JSON.stringify({ action: "calibrated", expectedUpdatedAt: lifecycleProfile.body.record.updatedAt, workOrderId: "WO-1047", evidenceRef: "CAL-CERT-API-001", note: "Reference probe comparison passed." }) });
  assert(technicianCalibration.response.ok && technicianCalibration.body.record.calibrationState === "current" && technicianCalibration.body.lifecycleEvent.actorId === "field-tech-show-suite", "Assigned technician calibration should persist evidence and attribution");
  const technicianRetireDenied = await fetchJson(`${baseUrl}api/device-lifecycle/${encodeURIComponent(lifecycleDevice.deviceId)}/actions`, { method: "POST", headers: { ...jsonHeaders("field-tech-show-suite"), "Idempotency-Key": "device-retire-denied" }, body: JSON.stringify({ action: "retired", expectedUpdatedAt: technicianCalibration.body.record.updatedAt, workOrderId: "WO-1047", note: "Technician may not retire assets." }) });
  assert(technicianRetireDenied.response.status === 403, "Technician should not retire a device");
  const technicianFault = await fetchJson(`${baseUrl}api/device-lifecycle/${encodeURIComponent(lifecycleDevice.deviceId)}/actions`, { method: "POST", headers: { ...jsonHeaders("field-tech-show-suite"), "Idempotency-Key": "device-fault-api-001" }, body: JSON.stringify({ action: "fault_reported", expectedUpdatedAt: technicianCalibration.body.record.updatedAt, workOrderId: "WO-1047", note: "Sensor drift returned during site check." }) });
  assert(technicianFault.response.ok && technicianFault.body.record.status === "fault" && technicianFault.body.record.registryStatus === "offline", "Technician fault report should isolate the device from active registry service");
  const staleDeviceAction = await fetchJson(`${baseUrl}api/device-lifecycle/${encodeURIComponent(lifecycleDevice.deviceId)}/actions`, { method: "POST", headers: { ...jsonHeaders("fm-lead"), "Idempotency-Key": "device-stale-api-001" }, body: JSON.stringify({ action: "returned_to_service", expectedUpdatedAt: technicianCalibration.body.record.updatedAt, note: "Stale page must not restore service." }) });
  assert(staleDeviceAction.response.status === 409 && staleDeviceAction.body.code === "DEVICE_LIFECYCLE_VERSION_CONFLICT", "Stale device action should not overwrite the field fault report");
  const returnedDevice = await fetchJson(`${baseUrl}api/device-lifecycle/${encodeURIComponent(lifecycleDevice.deviceId)}/actions`, { method: "POST", headers: { ...jsonHeaders("fm-lead"), "Idempotency-Key": "device-return-api-001" }, body: JSON.stringify({ action: "returned_to_service", expectedUpdatedAt: technicianFault.body.record.updatedAt, evidenceRef: "CAP-DEVICE-RETURN-001", note: "FM reviewed the connector replacement and stable reading." }) });
  assert(returnedDevice.response.ok && returnedDevice.body.record.status === "in_service" && returnedDevice.body.record.registryStatus === "active", "FM Lead should restore a reviewed device to service");
  const lifecycleHistory = await fetchJson(`${baseUrl}api/device-lifecycle/${encodeURIComponent(lifecycleDevice.deviceId)}/events`, { headers: principalHeaders("fm-lead") });
  assert(lifecycleHistory.response.ok && lifecycleHistory.body.events.length === 4 && lifecycleHistory.body.events[0].action === "returned_to_service", "Device lifecycle history should preserve profile, calibration, fault and restoration events");
  const storageAfterDeviceLifecycle = await fetchJson(`${baseUrl}api/storage`, { headers: principalHeaders("fm-lead") });
  assert(storageAfterDeviceLifecycle.body.deviceLifecycle.counts.profiles === 1 && storageAfterDeviceLifecycle.body.deviceLifecycle.counts.events === 4, "Device lifecycle storage health should expose managed profile and immutable events");
  assert(storageAfterDeviceLifecycle.body.deviceLifecycle.relationshipIntegrity.foreignKeyIssues === 0, "Device lifecycle introduced foreign-key issues");
  ...
  const staleMaintenanceCsv = "record_id,wall_id,service_date,status,priority,technician_id,tasks,notes,updated_at\nrec-good-001,MJ-HK-021,2026-08-29,Scheduled,high,field-tech-show-suite,Stale export,Older Airtable export,2026-08-28T09:00:00+08:00\n";
  const staleMaintenancePreview = await fetchJson(`${baseUrl}api/admin/imports/maintenance/preview`, { method: "POST", headers: jsonHeaders("fm-lead"), body: JSON.stringify({ filename: "airtable-maintenance-stale.csv", csv: staleMaintenanceCsv }) });
  assert(staleMaintenancePreview.response.status === 201 && staleMaintenancePreview.body.batch.validCount === 0 && staleMaintenancePreview.body.batch.invalidCount === 1 && staleMaintenancePreview.body.batch.errors[0].messages[0].includes("older than the existing work order"), "Older Airtable source versions should be blocked during preview");
  const staleMaintenanceApply = await fetchJson(`${baseUrl}api/admin/imports/maintenance/${encodeURIComponent(staleMaintenancePreview.body.batch.id)}/apply`, { method: "POST", headers: jsonHeaders("fm-lead"), body: "{}" });
  assert(staleMaintenanceApply.response.status === 409 && staleMaintenanceApply.body.code === "MAINTENANCE_IMPORT_HAS_ERRORS", "A stale Airtable preview should remain blocked at apply");
